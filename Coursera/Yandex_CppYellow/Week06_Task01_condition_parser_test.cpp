...
#include "Week06_Task01_condition_parser.h"
...
void TestParseCondition() {
    {
    istringstream is(""); // empty condition
    shared_ptr<Node> root = ParseCondition(is);
    Assert(root->Evaluate({ 2017, 1, 1 }, ""), "Parse condition 0.1");
    Assert(root->Evaluate({ 1, 1, 1 }, ""), "Parse condition 0.2");
    Assert(root->Evaluate({ 1, 1, 1 }, "event"), "Parse condition 0.3");
    }
    {
