...
namespace hrnk
{
   ...
   namespace ctci // Cracking the Code Interview tasks
   {
      ...
      {
         void rotate_one(std::vector<int>& v)
         {
            const int v0 = v[0];
            memmove(&v[0], &v[1], (v.size() - 1) * sizeof(int));
            v[v.size() - 1] = v0;
         }

         // CPU: O(N*k), memory: O(1)
         auto solution = [](std::vector<int>& v, int k) 
         {
            for (size_t i = 0; v.size() > 0 && i < k % v.size(); ++i)
            {
               rotate_one(v);
            }
         };

         // CPU: O(N*k), memory: O(N)
         ...
         void run(Solution solution, const string& input_string, bool output = true)
         {
